...
def evaluations_extend(origin_labels, predict_labels):
    ...
    try:
    	...
    	MCC = 0.0
    #AUC = plr.plot_roc(deci_value, origin_labels, output, title, roc)
...
def svm_train(arg1, arg2=None, arg3=None):
	...
	if param.cross_validation:
		...
		MCC, SN, SP = evaluations_extend(prob.y[:l], target[:l])
		if param.svm_type in [EPSILON_SVR, NU_SVR]:
			...
			print("Accuracy(ACC) = %g%%" % ACC)
			print("Mathew's Correlation Coefficient(MCC) = %g%%" % (MCC * 100))
			print("Sensitivity(Sn) = %g%%" % (SN * 100))
			print("Specificity(Sp) = %g%%" % (SP * 100))
