...
gnuplot_exe_list = [r'"C:\Program Files\gnuplot\pgnuplot.exe"', r'"..\..\gnuplot\bin\pgnuplot.exe"', r'".\gnuplot\bin\pgnuplot.exe"',  "/usr/bin/gnuplot","/usr/local/bin/gnuplot"]
#gnuplot_exe_list = [r'E:\Projects\libsvm-3.21\gnuplot\bin\pgnuplot.exe']
...
def plot_roc(deci, label, output, title, roc, roc_data_file):
    ...
    if roc:
        ...
        g.title = "ROC curve of %s (AUC = %.4f)" % (title,auc)
        #g.plotline(xy_arr)
        command = "plot '" + roc_data_file + "'" + " notitle with lines linewidth 1"
        g.plotline(command)
        #display on screen
        #s = gnuplot('onscreen')
        #s.xlabel = "False Positive Rate"
        #s.ylabel = "True Positive Rate"
        #s.title = "ROC curve of %s (AUC = %.4f)" % (title,auc)
        #s.plotline(xy_arr)
    return auc


#def plot_roc_cv(deci_list, label_list, output, title):
...
def main():
    ...
    if test_file:       #go with test_file
        ...
        plot_roc(deci, test_y, output_file, output_title, True)
    else:               #single file -> CV
        output_title = path.split(train_file)[1]
        deci = get_cv_deci(train_y, train_x, param, fold)
        plot_roc(deci, train_y, output_file, output_title, True, roc_data_file)
